...
void *mujer(){
	...
	sem_wait(&cola);											/*Esperamos a que haya gente en la Cola*/
	sem_wait(&smujeres); 										/*Esperamos a que puedan entrar los hombres*/
	if(personas == 0)											/*Si no hay nadie en el Lavabo*/
		sem_wait(&shombres);									/*Bloqueamos a las mujeres*/

	personas++;													/*Aumentamos las personas*/
	printf("Personas FM -----> %d\n",personas);
	sem_post(&smujeres);										/*Avisamos de que puede entrar los hombres*/
	sem_post(&cola);											/*Avisamos de que puede avanzar la Cola*/
	...
	sem_wait(&lavabo);													/*Controlamos el Lavabo*/
		...
		usleep(rand() % 50000);														/*Dormimos el proceso 2 Segundos, podeis ponerlo al gusto*/
