...
void cocheActivos(int i)	{ printf("El Coche %d está Activo\n",i);}
void PaseoCoche(int i)		{ printf("Los visitantes del coche %d estan paseando\n",i);}
void BajarCoche(int i)		{ printf("Se bajan 3 visitantes del coche %d\n",i);}
void SubirCoche(int i)		{ printf("Un visitante se montó en el coche %d\n",i);}
...
void *Coche(void * id){
	...
	int *idem,f, n = 0;
	idem = id;																				/*Pasamalos la id a idem*/
	...
	do{
		sem_wait(&mutex);																	/*Entramos en Sección Crítica (Para asignar el valor del coche del identificador a Coche Activo)*/
			...
			if( idC != -1){																	/*Si el coche está disponible*/
				...
				if(n<3){																	/*Si se ha llenado el coche y hay gente esperando...*/
